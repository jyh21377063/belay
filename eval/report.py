    ...
    for run_json in sorted(root.glob("*/*/*/run.json")):
        ...
        rows.append({
            ...
            **dict(zip(("f2p_passed", "f2p_total", "p2p_regressions"),
                       f2p_p2p(grade.get("rewards") if grade_mode == "replay" else rec.get("rewards")))),
...
def write(root: Path, rows: list[dict]) -> None:
    ...
    lines = [f"# {root.name}", "",
             "| benchmark | trials | resolved | 解决率 | 平均 Fix Rate | agent 用时中位数(min) | "
    ...
    lines += ["", "| 题目 | # | 结果 | Fix Rate | F2P 通过（不清零） | P2P 回归 | agent(min) | 总耗时(min) | 轮数 | "
    ...
    for r in rows:
        ...
        lines.append(f"| {r['task']} | {r['repeat']} | {mark} | {fmt(r['fix_rate'])} | {f2p} | {fmt(r.get('p2p_regressions'))} | "
