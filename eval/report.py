...
COLUMNS = ["task", "repeat", "status", "resolved", "fix_rate", "inline_resolved", "mismatch",
           ...
           "n_agent_steps", "summarization_count", "patch_files", "patch_loc", "test_files_changed", "error"]
...
def write(root: Path, rows: list[dict]) -> None:
    ...
    for r in rows:
        ...
        lines.append(f"| {r['task']} | {r['repeat']} | {mark} | {fmt(r['fix_rate'])} | {fmt(r['agent_min'])} | {total} | "
